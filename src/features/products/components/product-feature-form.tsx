...
import { useProductFeatureForm } from '../hooks/use-product-feature-form'
...
export const ProductFeatureForm: FC<ProductFeatureFormProps> = ({ initialValues }) => {
  ...
  const [createProductFeature] = useCreateProductFeatureMutation({
    refetchQueries: [GetProductFeaturesDocument],
  })
  const [updateProductFeature] = useUpdateProductFeatureMutation({
    refetchQueries: [GetProductFeaturesDocument],
  })
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useProductFeatureForm(initialValues)
  ...
  const onSubmit: SubmitHandler<FeatureFormValues> = async (formData) => {
    try {
      ...
      router.push(paths.products.features.root)
    } catch (error) {
