...
import { useSubfeatureForm } from '../../hooks/use-subfeature-form'
...
export const ProductSubfeatureForm: FC<ProductSubfeatureFormProps> = ({ initialValues }) => {
  ...
  const [createProductSubfeature, { loading: createLoading }] = useCreateProductSubFeatureMutation({
    refetchQueries: [GetProductSubFeaturesDocument],
  })
  const [updateProductSubfeature, { loading: updateLoading }] = useUpdateProductSubFeatureMutation({
    refetchQueries: [GetProductSubFeaturesDocument],
  })

  const { subfeatureForm, subfeatureItemForm, appendSubfeatureItemForm, removeSubfeatureItemForm } =
    useSubfeatureForm(initialValues)
  ...
  const onSubmit: SubmitHandler<SubfeatureFormValues> = async (formData) => {
    try {
      ...
      await router.push(paths.products.subfeatures.root)
    } catch (error) {
