import { Delete, ErrorOutline } from '@mui/icons-material'
import { IconButton } from '@mui/material'
import { Box } from '@mui/system'
import { FC, useState } from 'react'
import toast from 'react-hot-toast'

import { AlertDialog } from '@/shared/components/alert-dialog'
import { useIntl } from '@/shared/hooks/use-intl'
import {
  GetUserModulesByUserIdDocument,
  useDeleteUserModuleMutation,
} from '@/shared/graphql/generated/users-api'

interface UserModulesTableActionsProps {
  row: any
}

export const UserModulesTableActions: FC<UserModulesTableActionsProps> = (props) => {
  const {
    row: { idUser, idOu, idModule, name },
  } = props

  const [open, setOpen] = useState<boolean>(false)

  const intl = useIntl()

  const handleOpen = () => {
    setOpen(!open)
  }

  const [deleteUserModule] = useDeleteUserModuleMutation({
    refetchQueries: [GetUserModulesByUserIdDocument],
    onCompleted: () => {
      toast.success('Module deleted successfully', {
        position: 'top-right',
        duration: 2000,
      })
    },
  })

  const onDeleteConfirm = async () => {
    try {
      await deleteUserModule({
        variables: {
          data: {
            idUser,
            idModule,
            idOu,
          },
        },
      })
      handleOpen()
    } catch (error) {
      toast.error(`${error}`, {
        position: 'top-right',
        duration: 2000,
      })
    }
  }

  return (
    <>
      <AlertDialog
        open={open}
        maxWidth="sm"
        handleOpen={handleOpen}
        dialogTitle={intl.formatMessage('components.delete.alert.dialog.title', {
          type: 'User Module',
          name,
        })}
        dialogContent={intl.formatMessage('components.delete.alert.dialog.content')}
        onConfirm={onDeleteConfirm}
        icon={<ErrorOutline />}
      />
      <Box display="flex" gap="8px" justifyContent="center" width="100%">
        <IconButton onClick={handleOpen}>
          <Delete fontSize="small" />
        </IconButton>
      </Box>
    </>
  )
}
