...
export const CashierTransactionReservationsItemsSearchTableActions: FC<
...
> = (props) => {
  ...
  const onAddConfirm = async () => {
    ...
    if (
      ...
      !currencyExchangeRatesData?.getCurrencyExchangeRatesByDateAndIdOu?.length
    ) {
      toast.error(
    ...
    selectedRows.forEach((row) => {
        ...
        createCashierTransactionReservationItem({
          variables: {
            createCashierTransactionReservationItem: {
              ...
              amount3: Number(sub(row.item?.totalPriceUSD, row.item?.totalPayedUSD)),
      ...
      const dataProfit = profit.find(
        (itemProfit) =>
          itemProfit?.item?.rateInstance?.idRelatedRateInstance ===
          row.item.rateInstance.idRateInstance
      )
      if (dataProfit) {
        requests.push(
          createCashierTransactionReservationItem({
